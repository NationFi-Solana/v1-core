...
function main() {
  ...
  const walletKeypair = Keypair.fromSecretKey(
    Uint8Array.from(
      JSON.parse(fs.readFileSync('./key/my-keypair.json', 'utf8'))
    )
  );
  const wallet = new Wallet(walletKeypair);
  const provider = new AnchorProvider(connection, wallet);
  const program = getBettingProgram(
    provider,
    'GDWSP3B4CQ7yENdFcDQ7ScenHsMLQQ8SZHxCBzyTUZg8'
  );
...
async function initProgramState(program: Program<BettingDapp>) {
...
async function setWinner(program: Program<BettingDapp>) {
  ...
  const result = await program.methods
    ...
    .accounts({ programStateAccount: statePda })
    .rpc();
  return result;
}
