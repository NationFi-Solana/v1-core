...
export function getBetStatePDA({
  id,
  programId,
}: {
  id: number;
  ...
  const [BetStatePDA] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('be'), new Uint8Array(u16.buffer)],
    programId
  );
  return { BetStatePDA };
}

export function getUserSolPDA({
  id,
  programId,
  isBetA,
  user,
...
export function getBetFundsPDA({
  id,
  programId,
}: {
  id: number;
  ...
  const [BetFundsPDA] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from('program_funds'), new Uint8Array(u16.buffer)],
    programId
  );
  return { BetFundsPDA };
}
